...
export function TrustBar() {
  ...
  return (
    <section className="py-8 sm:py-10 bg-[#080c16] border-b border-border-subtle overflow-hidden relative transition-colors duration-300" id="partners">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mb-5">
        <p className="partner-title text-xs font-mono uppercase tracking-[0.2em] text-accent font-bold">
      ...
      <div className="relative w-full overflow-hidden py-2">
        {/* Edge Gradient Mask */}
        <div className="partner-edge-gradient-left absolute inset-y-0 left-0 w-16 sm:w-32 bg-gradient-to-r from-[#080c16] to-transparent z-10 pointer-events-none" />
        <div className="partner-edge-gradient-right absolute inset-y-0 right-0 w-16 sm:w-32 bg-gradient-to-l from-[#080c16] to-transparent z-10 pointer-events-none" />
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6 px-4 relative z-0">
          {partnerBadges.map((badge) => {
            ...
            return (
              <div
                key={badge.id}
                className={`partner-badge px-5 py-3 rounded-2xl bg-[#0f172a]/90 border border-slate-800/80 ${badge.badgeBg} transition-all duration-300 text-xs sm:text-sm font-semibold flex items-center gap-3 shadow-md hover:scale-105 group cursor-default`}
              >
                <LogoComponent className={`w-5 h-5 ${badge.color} transition-transform duration-300 group-hover:scale-110 shrink-0`} />
                <span className="tracking-wide font-semibold text-white group-hover:text-blue-400 transition-colors">
                  {badge.name}
                </span>
              </div>
            );
